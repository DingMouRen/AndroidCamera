package com.dingmouren.libcamera.view;
...
public class TouchControlView extends View {
    ...
    private static final String TAG = TouchControlView.class.getName();
    /**
     * 视频录制最长时间 秒
     */
    ...
    public TouchControlView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TouchControlView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void initData(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.TouchControlView);
        mMaxTime = a.getInt(R.styleable.TouchControlView_maxTime, VIDEO_RECORD_DEFAULT_MAX_TIME);
        mMinTime = a.getInt(R.styleable.TouchControlView_minTime, VIDEO_RECORD_DEFAULT_MIN_TIME);

        excicleMagnification = a.getFloat(R.styleable.TouchControlView_excicleMagnification
                , EXCICLE_MAGNIFICATION);
        innerCircleShrinks = a.getFloat(R.styleable.TouchControlView_excicleMagnification
        ...
        mInitExCircleRadius = mExCircleRadius = a.getDimension(R.styleable.TouchControlView_excircleRadius
                , VIDEO_RECORD_DEFAULT_EXCIRCLE_RADIUS);
        mInitInnerRadius = mInnerCircleRadius = a.getDimension(R.styleable.TouchControlView_innerCircleRadius
                , VIDEO_RECORD_DEFAULT_INNER_CIRCLE_RADIUS);

        mAnnulusColor = a.getColor(R.styleable.TouchControlView_annulusColor
                , VIDEO_RECORD_DEFAULT_ANNULUS_COLOR);
        mInnerCircleColor = a.getColor(R.styleable.TouchControlView_innerCircleColor
                , VIDEO_RECORD_DEFAULT_INNER_CIRCLE_COLOR);
        mProgressColor = a.getColor(R.styleable.TouchControlView_progressColor
    ...
    static class MHandler extends android.os.Handler {
        ...
        private WeakReference<TouchControlView> weakReference = null;

        public MHandler(TouchControlView controlView) {
            weakReference = new WeakReference<TouchControlView>(controlView);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            final TouchControlView touchControlView = weakReference.get();
            switch (msg.what) {
                case TouchControlView.MSG_START_LONG_RECORD:
                    if (touchControlView.onRecordListener != null) {
                        touchControlView.onRecordListener.OnRecordStartClick();
                    }
                    //内外圆动画，内圆缩小，外圆放大
                    touchControlView.startAnimation(touchControlView.mExCircleRadius,
                            touchControlView.mExCircleRadius * touchControlView.excicleMagnification,
                            touchControlView.mInnerCircleRadius,
                            touchControlView.mInnerCircleRadius * touchControlView.excicleMagnification);
                    break;
            }
        }
    }
